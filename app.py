...
이종호님, 날짜 범위를 건드려보시는 건 정말 탁월한 선택입니다! 네이버 쇼핑 인사이트 API는 실시간 데이터가 아니라 어제의 데이터도 오늘 오후 늦게나 업데이트되는 경우가 많거든요.

만약 오늘 날짜나 어제 날짜로 요청을 보내면 데이터가 아직 생성되지 않아 서버에서 **400 에러(형식 오류 또는 잘못된 요청)**를 뱉을 수 있습니다.

안전하게 3일 전부터 7일 전 사이의 데이터를 가져오도록 날짜 로직을 보강했습니다. 이번 코드는 400 에러 발생 시 **네이버가 보내는 진짜 이유(메시지)**를 화면에 바로 띄우도록 수정했으니, 만약 또 안 된다면 그 메시지만 알려주세요. 바로 해결해 드릴게요!

🛠️ 날짜 범위를 안전하게 조정한 app.py (무삭제 전체 코드)
Python
import streamlit as st
import requests
import json
...
# 2. API 설정 섹션
st.info("💡 데이터 업데이트 시차를 고려하여 3일 전 데이터를 기본으로 조회합니다.")
col1, col2 = st.columns(2)
with col1:
    c_id = st.text_input("Client ID", placeholder="네이버 Client ID 입력")
with col2:
    c_secret = st.text_input("Client Secret", type="password", placeholder="네이버 Client Secret 입력")

st.markdown("---")

# 3. 카테고리 설정 (모든 하위 카테고리 포함)
category_map = {
    "패션의류": {"여성의류": "50000000", "여성언더웨어/잠옷": "50000167", "남성의류": "50000001", "아동의류": "50000002"},
    "패션잡화": {"신발": "50000003", "가방": "50000004", "지갑": "50000005", "벨트": "50000006", "선글라스/안경테": "50000007", "헤어액세서리": "50000008"},
    "화장품/미용": {"스킨케어": "50000009", "메이크업": "50000010", "헤어케어": "50000011", "바디케어": "50000012", "향수": "50000013", "네일케어": "50000014"},
...
def generate_ai_titles(keyword):
    patterns = [
        f"아이랑 {keyword} 갈 때 무조건 챙겨야 할 필수템!",
        f"{keyword} 직접 다녀온 솔직 방문기 (주차, 명당 팁)",
        f"이번 주말 {keyword} 고민 중이라면? 아이가 정말 좋아해요",
        f"엄마들 사이에서 소문난 {keyword}, 직접 가보니 이유가 있네요"
    ]
    return random.sample(patterns, 3)
...
if st.button("🚀 심층 분석 시작"):
    if not c_id or not c_secret:
        ...
        with st.spinner('네이버 데이터를 수집 중입니다...'):
            if mode == "실시간 핫 키워드":
                success = False
                # 3일 전부터 10일 전까지 데이터를 역순으로 시도 (가장 최신 데이터 확보용)
                for day_offset in range(3, 11):
                    ...
                    if res.status_code == 200:
                        ...
                        # 에러 발생 시 원인 출력 (디버깅용)
                        try:
                            error_detail = res.json().get('message', '형식 오류')
                        except:
                            error_detail = "서버 응답 오류"
                        st.write(f"🔍 {target_date} 시도 결과: {res.status_code} ({error_detail})")
                
                if not success:
                    st.error("⚠️ 데이터 수집에 실패했습니다. API 권한(데이터랩 쇼핑인사이트) 설정과 날짜를 확인해주세요.")
            else:
