    ...
    if not api_key or not main_k:
        ...
        try:
            ...
            prompt_text = (
                ...
                "1. 제목추천 (상위노출용 3개)\n"
                "2. 요약문 (240~280byte)\n"
                "3. 본문 (3,500자 이상의 매우 상세한 내용)\n"
                "4. 추천해시태그 10개 (본문 하단에 배치)\n\n"
                "[원고 조건 - 엄격 준수]\n"
                f"- 키워드: '{main_k}' 4회, '{sub_k1}', '{sub_k2}', '{sub_k3}' 각 1회 자연스럽게 포함.\n"
                "- 말투: 무조건 30대 여성의 '반말 일기체' (~했음, ~했다, ~이다). 존댓말 절대 금지.\n"
                "- 소제목 형식: 반드시 '[그림이모지 1개 + 소제목]' 형식을 지킬 것.\n"
                "- 가독성: 한 줄에 60-70byte 내외로 짧게 끊기(모바일 최적화).\n"
                "- 이모티콘: 리스트 중 5~6개 필수 사용 (!(•̀ᴗ•́)و ̑̑ , (*ᴗ͈ˬᴗ͈)ꕤ*.ﾟ , (୨୧ ❛ᴗ❛)✧ , (୨୧ •͈ᴗ•͈) , (•̆ꈊ•̆ ) , (ꈍᴗꈍ)♡ , ̗̀ෆ(˶'ᵕ'˶)ෆ ̖́- , ٩(*•̀ᴗ•́*)و / 등).\n"
                "- SEO: 상위노출을 위해 정보성과 경험을 듬뿍 담아 3,500자 이상 아주 길게 작성.\n"
                f"- {image_instruction}\n"
                "- 금기: '원고 조건 확인' 같은 보고서용 텍스트는 절대 포함하지 마."
            )
            
            with st.spinner("정성 가득 담인 블로그 원고를 작성해드릴게요."):
