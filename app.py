    ...
    if not api_key or not main_k:
        ...
        try:
            ...
            prompt_text = (
                ...
                f"3. {image_instruction}\n\n"
                "[원고 조건 - 엄격 준수]\n"
                f"1. 키워드: '{main_k}' 4회, '{sub_k1}', '{sub_k2}', '{sub_k3}' 각 1회 자연스럽게 포함.\n"
                "2. 제목: 상위노출 될 수 있는 제목 3개 추천.\n"
                "3. 말투(매우 중요): 30대 여성의 '반말 일기체'로만 작성해. (~했음, ~했다, ~이다, 혼잣말 형식을 사용하고 '추천해요', '했어요', '더라구요' 같은 존댓말은 절대 쓰지 마.)\n"
                "4. 가독성: 한 줄에 공백포함 최대 60-70byte 내외로 끊어서 작성(모바일 최적화).\n"
                "5. 이모티콘: 리스트 중 5~6개 필수 사용 (!(•̀ᴗ•́)و ̑̑ , (*ᴗ͈ˬᴗ͈)ꕤ*.ﾟ , (୨୧ ❛ᴗ❛)✧ , (୨୧ •͈ᴗ͈) , (•̆ꈊ•̆ ) , (ꈍᴗꈍ)♡ , ̗̀ෆ(˶'ᵕ'˶)ෆ ̖́- , ٩(*•̀ᴗ•́*)و /, ٩( ᐢ-ᐢ ), / ٩(๑❛ᴗ❛๑)۶♡ , ٩(◕ᗜ◕)و , ദ്디( ¯꒳¯ ) , ☆٩(｡•ω<｡)﻿و , :) , :D , >_< , +ㅂ+ 등).\n"
...
if st.session_state.blog_script:
    ...
    # 💡 [하단 보고서 제거 로직 추가]
    # '원고 조건 확인'이나 '조건 적용 완료' 등의 키워드가 나오면 그 뒤는 다 잘라버립니다.
    ...
    if st.button("🖼️ 이미지만 추가/교체 생성", use_container_width=True):
        if not api_key or not image_requests:
            ...
            try:
                genai.configure(api_key=api_key)
                model = get_available_model()
                img_prompt = f"'{image_requests}'에 대한 상세한 Bing용 영어 이미지 프롬프트를 3개만 작성해줘. 서론 없이 프롬프트만."
